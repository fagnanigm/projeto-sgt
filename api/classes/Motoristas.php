...
class Motoristas {
	...
	public function get($args = array()){
		...
		// Count total
		$selectStatement = $this->db->select(array('COUNT(*) AS total'))->from('motoristas')->whereMany(array('active' => 'Y'),'=');
		...
		if($args['getall'] == '1'){
			...
			if($config['current_page'] <= $config['total_pages']){
				...
				$select = $this->db->query('SELECT * FROM motoristas WHERE active = \'Y\' ORDER BY create_time OFFSET '.$offset.' ROWS FETCH NEXT '.$config['item_per_page'].' ROWS ONLY');
				$response['results'] = $this->parser_fecth($select->fetchAll(\PDO::FETCH_ASSOC),'all');
