...
class IncluirOS {
	...
	public function insert($args = array()){
		...
		if(!isset($args['cCodIntOS'])){
			$response['error'] = 'O Código do Serviço é obrigatório.';
			return $response;
		}else{

			// checa se id do usuário existe
			$userSt = $this->db->select()->from('users')->whereMany(array('id' => $args['cCodIntOS'], 'active' => 'Y'), '=');
			$stmt = $userSt->execute();
	...
	public function get($args = array()){
		...
		if($args['getall'] == '1'){
			...
			$select = $this->db->query('SELECT * FROM cCodIntOS WHERE active = \'Y\' ORDER BY forma_nome');
