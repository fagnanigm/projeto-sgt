...
class PrazoRazoes {
	...
	public function insert($args = array()){
		...
		if(!isset($args['razao_nome'])){
			$response['error'] = 'O campo razao_nome é obrigatório.';
			return $response;
		} 
		...
		$insertStatement = $this->db->insert(array_keys($data))->into('autorizacao_servico_prazo_razao')->values(array_values($data));
	...
	public function get($args = array()){
		...
		// Count total
		$query_count = "SELECT COUNT(*) AS total FROM autorizacao_servico_prazo_razao WHERE active = 'Y'";

		// Filtro
		if(isset($args['razao_nome'])){
			$query_count .= " AND razao_nome LIKE '%".$args['razao_nome']."%'";
			$is_search = true;
		}
		...
		if($args['getall'] == '1'){
			...
			$select = $this->db->query('SELECT * FROM autorizacao_servico_prazo_razao WHERE active = \'Y\' ORDER BY razao_nome');
			...
			if($config['current_page'] <= $config['total_pages']){
				...
				$query = "SELECT * FROM autorizacao_servico_prazo_razao WHERE active = 'Y' ";

				// Filtro
				if(isset($args['razao_nome'])){
					$query .= " AND razao_nome LIKE '%".$args['razao_nome']."%' ";
				}

				// Config
				$query .= "ORDER BY razao_nome OFFSET ".$offset." ROWS FETCH NEXT ".$config['item_per_page']." ROWS ONLY";
	...
	public function delete($args = array()){
		...
		if(!isset($args['id'])){
			$response['error'] = 'ID da razão não especificado';
			return $response;
		}

		$updateStatement = $this->db->update(array('active' => 'N'))->table('autorizacao_servico_prazo_razao')->where('id', '=', $args['id']);
	...
	public function update($args = array()){
		...
		$updateStatement = $this->db->update()->set($data)->table('autorizacao_servico_prazo_razao')->whereMany( array('id' => $id), '=');
	...
	public function get_by_id($id = false, $args = false){
		...
		$selectStatement = $this->db->select()->from('autorizacao_servico_prazo_razao')->whereMany(array('id' => $id, 'active' => 'Y' ), '=');
