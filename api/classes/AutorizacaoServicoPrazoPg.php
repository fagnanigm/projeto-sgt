...
class AutorizacaoServicoPrazoPg {
	...
	public function insert($args = array()){
		...
		if(!isset($args['prazo_nome'])){
			$response['error'] = 'O campo prazo_nome é obrigatório.';
			return $response;
		} 
		...
		$insertStatement = $this->db->insert(array_keys($data))->into('autorizacao_servico_prazo_pg')->values(array_values($data));
	...
	public function get($args = array()){
		...
		// Count total
		$query_count = "SELECT COUNT(*) AS total FROM autorizacao_servico_prazo_pg WHERE active = 'Y'";

		// Filtro
		if(isset($args['prazo_nome'])){
			$query_count .= " AND prazo_nome LIKE '%".$args['prazo_nome']."%'";
			$is_search = true;
		}
		...
		if($args['getall'] == '1'){
			...
			$select = $this->db->query('SELECT * FROM autorizacao_servico_prazo_pg WHERE active = \'Y\' ORDER BY prazo_nome');
			...
			if($config['current_page'] <= $config['total_pages']){
				...
				$query = "SELECT * FROM autorizacao_servico_prazo_pg WHERE active = 'Y' ";

				// Filtro
				if(isset($args['prazo_nome'])){
					$query .= " AND prazo_nome LIKE '%".$args['prazo_nome']."%' ";
				}

				// Config
				$query .= "ORDER BY prazo_nome OFFSET ".$offset." ROWS FETCH NEXT ".$config['item_per_page']." ROWS ONLY";
	...
	public function delete($args = array()){
		...
		$updateStatement = $this->db->update(array('active' => 'N'))->table('autorizacao_servico_prazo_pg')->where('id', '=', $args['id']);
	...
	public function update($args = array()){
		...
		$updateStatement = $this->db->update()->set($data)->table('autorizacao_servico_prazo_pg')->whereMany( array('id' => $id), '=');
	...
	public function get_by_id($id = false, $args = false){
		...
		$selectStatement = $this->db->select()->from('autorizacao_servico_prazo_pg')->whereMany(array('id' => $id, 'active' => 'Y' ), '=');
