...
class TaxasLicencasTipos {
	...
	public function insert($args = array()){
		...
		$insertStatement = $this->db->insert(array_keys($data))->into('taxas_licencas_tipos')->values(array_values($data));
	...
	public function get($args = array()){
		...
		// Count total
		$query_count = "SELECT COUNT(*) AS total FROM taxas_licencas_tipos WHERE active = 'Y'";
		...
		if($args['getall'] == '1'){
			...
			$select = $this->db->query('SELECT * FROM taxas_licencas_tipos WHERE active = \'Y\' ORDER BY tipo_nome');
			...
			if($config['current_page'] <= $config['total_pages']){
				...
				$query = "SELECT * FROM taxas_licencas_tipos WHERE active = 'Y' ";
	...
	public function delete($args = array()){
		...
		$updateStatement = $this->db->update(array('active' => 'N'))->table('taxas_licencas_tipos')->where('id', '=', $args['id']);
	...
	public function update($args = array()){
		...
		$updateStatement = $this->db->update()->set($data)->table('taxas_licencas_tipos')->whereMany( array('id' => $id), '=');
	...
	public function get_by_id($id = false, $args = false){
		...
		$selectStatement = $this->db->select()->from('taxas_licencas_tipos')->whereMany(array('id' => $id, 'active' => 'Y' ), '=');
