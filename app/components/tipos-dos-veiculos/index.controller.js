(function () {
    ...
    angular
        .module('app')
        .controller('TiposDosVeiculos.IndexController', Controller);

    function Controller($rootScope,$scope,$http,$location,ngToast,$localStorage,GlobalServices) {
        
        $scope.results = {};

        $scope.currentPage = ($rootScope.$state.name == 'tipos-dos-veiculos-paged' ? $rootScope.$stateParams.page : '1' );

        $scope.get_results = function(){

            $rootScope.is_loading = true;

            var rest_address = '/api/public/veiculos/tipos/get';
        ...
        $scope.delete_item = function(id){
            if(confirm("Deseja excluir esse tipo do veículo?")){
                $rootScope.is_loading = true;
                $http.post('/api/public/veiculos/tipos/delete',{ id : id }).then(function (response) {
                    $scope.get_results();

                    ngToast.create({
                        className: 'success',
                        content: 'Tipo do veículo excluído com sucesso'
                    });
